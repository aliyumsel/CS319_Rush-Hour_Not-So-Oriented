...
public class SettingsPanel extends JPanel {
    ...
    private void createComponents() {
        ...
        if (PlayerManager.instance.getCurrentPlayer().settings.getControlPrefrence().equals("Slide"))
...
//        keyboard.setBounds(645, 175, mouse.getPreferredSize().width, mouse.getPreferredSize().height);
    ...
    private void updateThemeButtons() {
        ...
        if (!themeManager.classic.isUnlocked()) {
            classic.setIcon(new ImageIcon(classicD));
            classic.setRolloverIcon(new ImageIcon(classicD));
        } else {
            classic.setIcon(new ImageIcon(classicImage));
            classic.setRolloverIcon(new ImageIcon(classicHighlightedImage));
        }

        if (!themeManager.safari.isUnlocked()) {
            safari.setIcon(new ImageIcon(safariD));
            safari.setRolloverIcon(new ImageIcon(safariD));
        } else {
            safari.setIcon(new ImageIcon(safariImage));
            safari.setRolloverIcon(new ImageIcon(safariHighlightedImage));
        }

        if (!themeManager.space.isUnlocked()) {
            space.setIcon(new ImageIcon(spaceD));
            space.setRolloverIcon(new ImageIcon(spaceD));
        } else {
            space.setIcon(new ImageIcon(spaceImage));
            space.setRolloverIcon(new ImageIcon(spaceHighlightedImage));
        }
    }
